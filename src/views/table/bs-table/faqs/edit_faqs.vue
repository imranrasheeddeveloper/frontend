...
export default {
  ...
  methods: {
    ...
    getFaq(id) {
      axios
        .get(`/faqs/getFAQs_by_id/${id}`)
        .then(response => {
          if (response.data.hasOwnProperty('success')) {
            if (response.data.success === true) {
              console.log(response.data.data)
              this.formValues.question = response.data.data.question
              this.formValues.answer = response.data.data.answer
              this.formValues.id = response.data.data.id
            } else {
              this.isSubmitting = false
